...
func main() {
	// Open the CSV file containing the list of repos
	...
	outputFile, err := os.Create("contributors_output.csv")
	if err != nil {
	...
	writer.Write([]string{"Repo Name", "Repo Link", "Contributor Name", "Contributor Email"})
	...
	for i, record := range records {
		...
		repoLink := fmt.Sprintf("https://github.com/amex-eng/%s", repoName)
		...
		// For each contributor, get their email and write to CSV
		for _, contributor := range contributors {
			...
			writer.Write([]string{repoName, repoLink, contributor, email})
		}
	}

	fmt.Println("CSV file 'contributors_output.csv' has been created with repo details and contributors.")
}
