...
// SearchHierarchy searches for the user's info and recursively searches for the manager hierarchy up to 3 levels.
func SearchHierarchy(l *ldap.Conn, email string, level int) error {
    if level >= 3 {
        return nil // Stop after 3 levels
    }
    ...
    personName := strings.Split(email, "@")[0] // Extract name from email (part before '@')
    ...
    fmt.Printf("%s%s's Email: %s\n", indent, personName, userEmail)

    // If manager email is not empty, recursively search for the manager's hierarchy
    if managerEmail != "" {
        // Format manager's name based on the manager's email
        managerName := strings.Split(managerEmail, "@")[0] // Extract name from email
