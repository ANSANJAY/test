...
func getWorkflowStatus(repo string) (string, string, error) {
	...
	// Check if the output is empty, indicating no runs were found (i.e., not triggered)
...
func main() {
	...
	for i, repo := range repos {
		...
		go func(i int, repo string) {
			...
			// Fetch workflow status (trigger status and build status)
