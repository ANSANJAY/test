...
func getPRAge(repo string) (string, error) {
	...
	cmd := exec.Command("gh", "pr", "list", "--repo", fmt.Sprintf("%s/%s", owner, repo), "--state", "open", "--json", "title,createdAt")
	output, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("error running command: %v", err)
	}

	// Debugging: Print the output of the gh command
	fmt.Printf("Debug: Output of gh command for repo %s:\n%s\n", repo, string(output))
	...
	for _, pr := range prs {
		if pr.Title == prTitle {
			if pr.CreatedAt == "" {
				return "creation date missing", nil
			}
